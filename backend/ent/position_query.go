...
func (pq *PositionQuery) QueryEmployee() *EmployeeQuery {
...
func (pq *PositionQuery) WithEmployee(opts ...func(*EmployeeQuery)) *PositionQuery {
	query := &EmployeeQuery{config: pq.config}
...
func (pq *PositionQuery) sqlAll(ctx context.Context) ([]*Position, error) {
	...
	if query := pq.withEmployee; query != nil {
		...
		query.Where(predicate.Employee(func(s *sql.Selector) {
			s.Where(sql.InValues(position.EmployeeColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.position_employee
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "position_employee" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "position_employee" returned %v for node %v`, *fk, n.ID)
			}
