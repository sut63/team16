...
func (cq *CourseQuery) QueryBookcourse() *BookcourseQuery {
	query := &BookcourseQuery{config: cq.config}
...
func (cq *CourseQuery) WithBookcourse(opts ...func(*BookcourseQuery)) *CourseQuery {
	query := &BookcourseQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withBookcourse = query
	return cq
}
...
func (cq *CourseQuery) sqlAll(ctx context.Context) ([]*Course, error) {
	...
	if query := cq.withBookcourse; query != nil {
		...
		query.Where(predicate.Bookcourse(func(s *sql.Selector) {
			s.Where(sql.InValues(course.BookcourseColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.course_bookcourse
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "course_bookcourse" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
